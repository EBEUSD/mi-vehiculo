        ...
        return (
          <article key={vehicle.id} className={styles.card}>
            <Link
              ...
              state={{ fromSearch: currentSearch ? `/vehiculos?${currentSearch}` : "/vehiculos" }}
              className={styles.cardLink}
            >
              <div className={styles.imageWrap}>
                <img src={vehicle.image} alt={vehicle.title} loading="lazy" />
                <span className={`${styles.tag} ${vehicle.tag === "DESTACADO" ? styles.green : vehicle.tag === "NUEVO" ? styles.blue : styles.gray}`}>
                  {vehicle.tag}
                </span>
                <button
              ...
              <div className={styles.info}>
                <h3>{vehicle.title}</h3>
                <strong>{formatUSD(vehicle.price)}</strong>
                <p>{vehicle.year} · {formatKm(vehicle.km)} km · {vehicle.fuel} · {vehicle.transmission}</p>
                <span>{vehicle.location}</span>
                {showRemove && (
                  <div className={styles.favoriteActions}>
                    <button className={styles.removeBtn} onClick={(e) => handleRemove(e, vehicle.id)}>
                      <FiTrash2 /><span>Quitar</span>
                    </button>
                  </div>
                )}
              </div>
